...
function VideoReviewCard({ review, t, className = '' }) {
    ...
    const togglePlayPause = async () => {
        ...
        if (isPlaying) {
            ...
            try {
                setShowPoster(false); // Скрываем постер при первом запуске
                await videoRef.current.play();
                setIsPlaying(true);
            } catch (err) {
                console.error('Не удалось запустить видео:', err);
    ...
    const handleVideoEnded = () => {
        setIsPlaying(false);
        setShowPoster(true); // Показываем постер обратно
    };
    ...
    return (
        <div className={`bg-white rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 ${className}`}>
            ...
            <div className="relative h-[70vh] lg:h-[500px] bg-gray-100">
                ...
                {/* Имя и текст */}
                {(review.name || review.preview) && (
                    <div className="absolute bottom-6 left-6 right-6">
                        {review.name && <h3 className="text-2xl font-bold text-white mb-1">{review.name}</h3>}
                        {review.preview && <p className="text-white/80 text-sm leading-tight">{review.preview}</p>}
                    </div>
                )}
            </div>

            {/* Круглое видео с оптимизированным постером */}
            <div className="relative -mt-20 pb-8 flex justify-center px-6">
                <div className="relative">
                    <div className="w-64 h-64 rounded-full overflow-hidden shadow-2xl ring-8 ring-white bg-black">
                        {/* Оптимизированный постер через Next.js Image */}
                        {showPoster && (
